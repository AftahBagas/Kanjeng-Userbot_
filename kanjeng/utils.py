...
from kanjeng import CMD_LIST, LOAD_PLUG, SUDO_LIST, bot
from kanjeng.Configs import Config
from kanjeng.wraptools import (
    am_i_admin,
    ignore_bot,
    ignore_fwd,
    ignore_grp,
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        import kanjeng.modules
        import kanjeng.utils

        path = Path(f"kanjeng/modules/{shortname}.py")
        name = "kanjeng.modules.{}".format(shortname)
        ...
        # support for kanjeng userbot
        sys.modules["kanjeng.util"] = kanjeng.utils
        sys.modules["kanjeng.util"] = kanjeng.utils
        sys.modules["userbot.utils"] = kanjeng.utils
        sys.modules["userbot.plugins"] = kanjeng.modules
        sys.modules["plugins"] = kanjeng.modules
        sys.modules["userbot"] = kanjeng
        mod.Config = Config
        ...
        sys.modules["kanjeng.events"] = kanjeng.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["kanjeng.modules." + shortname] = mod
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"bakauserbot.modules.{shortname}"
...
def kanjeng_on_cmd(pattern=None, **args):
    args["func"] = lambda e: e.via_bot_id is None
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            ftext += "--------BEGIN KANJENG USERBOT TRACEBACK LOG--------"
            ftext += "\nDate: " + date
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"kanjeng/modules/assistant/{shortname}.py")
        name = "kanjeng.modules.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        spec.loader.exec_module(mod)
        sys.modules["bakauserbot.modules.assistant" + shortname] = mod
